...
function tooltips() {
    function t(t) {
        ...
        e.create(n, t.currentTarget), e.position(n, t.currentTarget)
    }

    function e() {
        n.className = "tooltip-container no-display", n.removeChild(n.firstChild), n.removeAttribute("style")
    }
    ...
    function o() {
        ...
        Array.prototype.forEach.call(o, function (o) {
            if (!o.classList.contains('init')) {
                o.classList.add('init');
                o.addEventListener("mouseover", t, !1), o.addEventListener("mouseout", e, !1)
            }
        })
    }
    ...
    var n, i, l, r = {
        ...
        position: function (t, e) {
            var o, n = i.top - i.height - 120;
            i.left > window.innerWidth - 100 ?
                (t.style.left = i.left - 220 + "px",
...
var Helpers = {
    ...
    find: function (elem, selector) {
        ...
        switch (typeof elem) {
            case 'string' :
                outers = document.querySelectorAll(elem);
                break;
            case 'object' :
                Array.prototype.push.apply(outers, [elem]);
        }
        ...
        try {
            ...
            if (found_elements.length == 0) {
                console.error('Cannot find ' + selector + ' in ' + elem);
    ...
    checkboxes: function (checkbox) {
        var checkbox = checkbox.target ? checkbox.target : checkbox;
        var filter = Helpers.parents(checkbox, '.filter')[0];
        if (checkbox.checked) {
            filter.classList.add('filter_checked')
        } else {
            filter.classList.remove('filter_checked')
        }
    },
    radio: function (radio) {
        var radio = radio.target ? radio.target : radio;
        var inputs = document.getElementsByClassName('filter__input-r');
        var filter = Helpers.parents(radio, '.filter')[0];
        for (var i = 0; i < inputs.length; i++) {
            if (inputs[i].name == radio.name) {
                Helpers.parents(inputs[i], '.filter')[0].classList.remove('filter_checked')
            }
        }
        if (radio.checked) {
            filter.classList.add('filter_checked')
...
var lsFusion = {
    init: function () {
        ...
        if (tabs.length > 1) {
            table = tabs[0].querySelectorAll('.table__wrapp > .table')[0];
            lsFusion.changeTab(table);
            if (table) {
                setTimeout(lsFusion.table.colheight(), 50);
            }
        }
        ...
        lsFusion.lablebreak();
        lsFusion.filtering();
        lsFusion.radioselect();
        lsFusion.checkboxselect();
        lsFusion.closeMenu();
        lsFusion.editor();
        if (table) {
            setTimeout(lsFusion.table.colheight(table), 50);
            setTimeout(lsFusion.table.colheight(table), 100);
            setTimeout(lsFusion.table.fixhead(table), 100);
            setTimeout(lsFusion.table.fixcol(table), 100);
            setTimeout(lsFusion.table.fixcorner(table), 1000);
            lsFusion.table.filter(table);
            lsFusion.table.target();
        ...
        for (var i = 0; i < btns_t.length; i++){
            btns_t[i].addEventListener('click', op_t, false);
        }
        function op_t(){
            var tool = this.querySelector('.tooltip');
            if (!event.target.classList.contains('close') && !event.target.parentNode.classList.contains('close') && !event.target.parentNode.parentNode.classList.contains('close')){
                tool.classList.add('opened');
            }
            return false;
        }
        for (var i = 0; i < btns_close.length; i++){
            btns_close[i].addEventListener('click', op_cl, false);
        }
        function op_cl(){
            var tool = this.parentNode;
            tool.classList.remove('opened');
            return false;
        }
    },
    changeTab: function (table) {
        var tabs_btn = document.querySelectorAll('.mode-panel__mode-item'),
            tabs = document.querySelectorAll('.mode-content');
            ...
            for (var i = 0; i < tabs.length; i++) {
                ...
                if (tab_id == activeTabId) {
                    ...
                    if (table) {
                        ...
                        setTimeout(lsFusion.table.fixcol(table), 50);
    ...
    closeMenu: function () {
        var body = document.querySelector("body"),
            check = document.getElementById('nav-trigger'),
            flag = 0;
    ...
    toggleMenu: function () {
        var btn_open = document.getElementsByClassName('btn-menu')[0],
            btn_close = document.getElementsByClassName('btn-menu-x')[0],
            check = document.getElementById('nav-trigger'),
            body = document.getElementsByTagName('body')[0];

        document.addEventListener("click", function (event) {
            var checked = check.checked;
            if (event.target.closest(".menu-opened") && !event.target.closest(".head_block") && !event.target.closest(".mobile-menu__panel") && !event.target.closest(".btn-menu-x, .btn-menu")) {
                body.classList.remove('menu-opened');
                check.checked = false;
            }
            if (event.target.closest(".btn-menu-x, .btn-menu")) {
                if (body.classList.contains('menu-opened')) {
                    body.classList.remove('menu-opened');
                } else {
                    body.classList.add('menu-opened');
                }
            } else if (checked && event.target.closest(".mobile-menu__panel") && !event.target.closest(".btn-menu-x, .btn-menu")) {
                return false;
            } else if (checked && event.target.closest(".menu-opened") && !event.target.closest(".btn-menu-x, .btn-menu")) {
                //console.log(event.target.closest(".menu-opened"))
            }
        });
    },
    droplist: function () {
        var fph = document.getElementsByClassName('filter-panel__current');
        for (var i = 0; i < fph.length; i++) {
            fph[i].addEventListener('click', addClassActive, false);
            fph[i].classList.add('ssss')
        }

        function addClassActive() {
            var menu = Helpers.parents(this, '.filter-panel')[0];
    ...
    lablebreak: function (tab) {
        ...
        if (tab) {
            var labelContainer = tab.getElementsByClassName('filter-panel__list');
            if (labelContainer.length == 0) {
                return;
            }
            var labelItem = Helpers.find(labelContainer[0], '.filter-panel__item'),
                labels_w = 0;
        } else {
            var labelContainer = document.getElementsByClassName('filter-panel__list');
            if (labelContainer.length == 0) {
                return;
            }
            var labelItem = Helpers.find(labelContainer[0], '.filter-panel__item'),
        ...
        function removeClass(el) {
            ...
            el.classList.remove('last-in-line');
            el.classList.remove('line-first');
            el.classList.remove('line-last');
            for (var i = 0; i < el.classList; i++) {
                if (el.classList[i].startsWith('line-')) {
        ...
        window.onresize = function () {
            if (window.matchMedia('(min-width: 627px)').matches) {
                ...
                for (var i = 0; i < labelItem.length; i++) {
                    removeClass(labelItem[i]);
                }
            }
        };

        function tabIniter() {
            var labelContainer_w = labelContainer[0].offsetWidth,
                label_line = 0,
                line_count = [1];

            for (var i = 0; i < labelItem.length; i++) {
                var line_num = line_count[line_count.length - 1],
                    line_count_prev = line_num;
                removeClass(labelItem[i]);
                labels_w = labelItem[i].offsetWidth;
                label_line += labels_w - 1;
                if (label_line > labelContainer_w && i > 0) {
                    labelItem[i].classList.add('line-' + (line_num + 1));
                    labelItem[i].classList.add('first-in-line');
                    labelItem[i - 1].classList.add('last-in-line');
                    label_line = labels_w;
                    line_num++;
                    if (line_count_prev != line_num) {
                        Array.prototype.push.apply(line_count, [line_num]);
                    }
                } else {
                    if (line_count.length == 1) {
                        labelItem[i].classList.add('line-first');
                    } else {
                        labelItem[i].classList.add('line-' + line_num);
                    }
                    labelItem[i].classList.remove('first-in-line');
                }
            }

            if (line_count.length > 2) {
                var last_lines_class = 'line-' + line_count[line_count.length - 1];
                var last_lines = document.getElementsByClassName(last_lines_class);
                lastLineClass();
                function lastLineClass() {
                    last_lines[last_lines.length - 1].classList.add('line-last');
                    last_lines[last_lines.length - 1].classList.remove(last_lines_class);
                    if (last_lines[0]) {
                        lastLineClass();
                    }
                }
    ...
    checkboxselect: function (checkbox) {
        var checkbox = checkbox ? checkbox : document.getElementsByClassName('filter__input');
        for (var i = 0; i < checkbox.length; i++) {
            Helpers.checkboxes(checkbox[i]);
            checkbox[i].addEventListener('change', Helpers.checkboxes, false);
        }
    },
    radioselect: function () {
        var radio = document.getElementsByClassName('filter__input-r');
        //Helpers.radio();
        for (var i = 0; i < radio.length; i++) {
            radio[i].addEventListener('change', Helpers.radio, false);
        }
    },
    filtering: function () {
        var filter_cont_active = document.querySelectorAll('.conseption-content');
        if (filter_cont_active.length > 0) {
            filterActive();
            var btns = document.querySelectorAll('[data-sort-selector]');
            for (var i = 0; i < btns.length; i++) {
                btns[i].addEventListener('change', filterActive, false);
            }
        }

        function filterActive() {
            var param = collectParamActive();
            if (param.length > 0) {
                filter_cont_active[0].classList.add('disable');
                compareFilterActive(param);
            } else {
                var items = document.querySelectorAll('[data-sort-name]');
                filter_cont_active[0].classList.remove('active');
        ...
        function collectParamActive() {
            var filters = document.querySelectorAll('[data-sort-selector]'),
                filter_param = [];

            for (var i = 0; i < filters.length; i++) {
                if (filters[i].checked) {
                    var sort = filters[i].dataset.sortSelector;
                    Array.prototype.push.apply(filter_param, [sort]);
                }
            }
            return filter_param;
        }

        function compareFilterActive(param) {
            var items = document.querySelectorAll('[data-sort-name]');
            console.log(items)
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                for (var k = 0; k < param.length; k++) {
                    var check = param[k],
                        params = items[i].dataset.sortName,
                        re = new RegExp('(^|\\s)' + check + '(?=\\s|$)', 'g');
                    if (check == 'all') {
                        filter_cont_active[0].classList.remove('disable');
                        items[i].classList.remove('active');
                        break;
                    } else if (re.test(params) == true) {
        ...
        var elem = document.querySelector('.filtering');
        if (elem) {
            var mixer = mixitup(elem, {
                selectors: {
                    target: '.filtering-item'
                },
                animation: {
                    duration: 500
                },
        ...
        var filter_cont = document.querySelectorAll('.filtering');
        if (filter_cont.length > 0) {
            filterSort();
            var btns = document.querySelectorAll('[data-sort-selector]');
            for (var i = 0; i < btns.length; i++) {
                btns[i].addEventListener('change', filterSort, false);
            }
        }
        function filterSort() {
            if (typeof this != 'undefined'){
                var cl = this.dataset.sortSelector;
            }
            var param = collectParamSort();
            if (param.length > 0 && def_init) {
                console.log(def_init+'param')

                compareFilterSort(param, cl);
                /*setTimeout(function(){
                 var items = document.querySelectorAll('.filtering-item');
                 var max = 1;
                 for (var i = 0; i < items.length; i++) {
                 if (items[i].dataset.priority > max){
                 max = items[i].dataset.priority;
                 }
                 if (items[i].classList.contains('enabled')){
                 //items[i].dataset.priority = max + 1;
                 console.log(items[i].dataset.sortName)
                 }
                 //console.log(items[i].dataset.sortName, items[i].dataset.priority)
                 }
                 }, 100)*/
                mixer.sort('enable:asc priority:desc');

            } else {
                console.log(def_init+'def')

                var items = document.querySelectorAll('.filtering-item');
                for (var i = 0; i < items.length; i++) {
                    items[i].classList.remove('item_disabled-filter');
                    items[i].classList.add('enabled');
                    items[i].setAttribute('data-enable', false);
                    items[i].dataset.priority = items[i].dataset.default;
                }
                mixer.sort('enable:asc priority:asc default:asc');
            }
            if (!def_init){
                console.log(def_init+'def_in')
                var items_s = document.querySelectorAll('.filtering-item');
                for (var k = 0; k < items_s.length; k++) {
                    items_s[k].dataset.default = k;
                    if (items_s.length-1 == k){
                        compareFilterSort(param, cl);
                        mixer.sort('enable:asc priority:asc default:asc');
                        def_init = 1;
                    }
        ...
        var max = 1;
        var def = 0;
        function compareFilterSort(param, sort) {
            var items = document.querySelectorAll('.filtering-item');
            for (var i = 0; i < items.length; i++) {
                if (typeof  items[i].dataset.priority == 'undefined') {
                    items[i].dataset.priority = 1;
                }
                if (typeof  items[i].dataset.default == 'undefined') {
                    def++;
                    items[i].dataset.default = def;
                }
                if (items[i].dataset.priority > max){
                    max = items[i].dataset.priority;
                }
                for (var k = 0; k < param.length; k++) {
                    ...
                    if (check == 'for_all') {
                        ...
                        if (check == sort){
                            items[i].dataset.priority = parseFloat(max) + 1;
                        } else {
                            items[i].dataset.priority = parseFloat(max);
                        }
                        items[i].classList.add('enabled');
                        items[i].classList.remove('item_disabled-filter');
                        items[i].setAttribute('data-enable', false);
                        ...
                        items[i].classList.add('item_disabled-filter');
                        items[i].classList.remove('enabled');
                        items[i].setAttribute('data-enable', true);
                        items[i].dataset.priority = 1;
                        //items[i].dataset.priority = parseFloat(items[i].dataset.priority) - 1;
                    }
                }
            }
        }
    ...
    table: {
        filter: function (table) {
            var table = table ? table : document.querySelectorAll('.table')[0],
                tab = Helpers.parents(table, '.mode-content')[0];
            if (table) {
                setTimeout(filter, 100);
                var checkbox = tab.getElementsByTagName('input');
                for (var i = 0; i < checkbox.length; i++) {
                    checkbox[i].addEventListener('change', filter, false);
                }
            }
            ...
            function filter() {
                ...
                if (param.length > 0) {
                    ...
                    var rows = tab.querySelectorAll('[data-row-type]');
            ...
            function collectParam() {
                var filter_panel = tab.querySelectorAll('.filter-panel'),
                    filter_param = [];
                if (filter_panel[0]) {
                    var checkboxes = filter_panel[0].querySelectorAll('input');
                } else {
                    checkboxes = 0;
                }
                ...
                for (var i = 0; i < checkboxes.length; i++) {
                    if (checkboxes[i].checked) {
                        ...
                        Array.prototype.push.apply(filter_param, [id]);
                    }
                }
            ...
            function compareFilter(param) {
                var rows = table.querySelectorAll('[data-row-type]');
                for (var i = 0; i < rows.length; i++) {
                    for (var k = 0; k < param.length; k++) {
                        var check = param[k],
                            params = rows[i].dataset.rowType,
                            re = new RegExp('(^|\\s)' + check + '(?=\\s|$)', 'g');
                        if (re.test(params) == true) {
                            rows[i].classList.remove('disabled');
                            var table_body = tab.querySelectorAll('.table__head + .table__body:not(.tb_cloned)');
                            var table_body_c = tab.querySelectorAll('.table__head + .table__body.tb_cloned');
                            var first_el = table_body[0].querySelectorAll('[data-row-type]');
                            var first_el_c = table_body_c[0].querySelectorAll('[data-row-type]');
                            if (rows[i].childNodes.length > 1) {
                                table_body[0].insertBefore(rows[i], first_el[0]);
                            } else if (rows[i].childNodes.length == 1) {
                                table_body_c[0].insertBefore(rows[i], first_el_c[0]);
                            }
                            break;
                        } else {
        ...
        fixhead: function (table) {
            var table = table ? table : document.getElementsByClassName('table')[0],
                th = table.getElementsByClassName('table__head')[0],
                tab = Helpers.parents(table, '.mode-content')[0];

            if (typeof th !== 'undefined') {
                var th_cln = th.cloneNode(true);
                th_cln.classList.add('th_cloned');
                if (table.querySelectorAll('.th_cloned').length == 0) {
                    table.appendChild(th_cln);
                }
                var th_c = tab.getElementsByClassName('th_cloned')[0],
                    th_cells = Helpers.find(th, '.table__cell'),
                    th_c_cells = Helpers.find(th_c, '.table__cell');

                for (var i = 0; i < th_cells.length; i++) {
                    th_c_cells[i].style.width = parseFloat(th_cells[i].getBoundingClientRect().width) + 'px';
                }
                window.onscroll = function () {

                    var th_bound = th.getBoundingClientRect(),
                        table_bound = table.getBoundingClientRect();
                    var yy = Math.abs(table_bound.y);

                    if (!yy) {
                        yy = Math.abs(table_bound.top)
                    }
                    var th_y = th_bound.y;
                    if (!th_y) {
                        th_y = th_bound.top;
                    ...
                    if (th_y < 0 && pageYOffset < table_bound.height + table_bound.top + pageYOffset - th_bound.height) {
                        th_c.classList.add('fix');
                        th_c.style.width = table_bound.width + "px";
                        th_c.style.top = yy + 'px';
                    } else {
                        th_c.classList.remove('fix');
                    }

                    var corner = tab.getElementsByClassName('table__corner');
                    if (corner.length > 0) {
                        var yy = table_bound.y * -1;
                        if (!th_y) {
                            yy = table_bound.top * -1;
                        }
                        if (yy < 0) {
                            yy = 0
                        } else if (yy > table_bound.height - corner[0].getBoundingClientRect().height) {
                            yy = table_bound.height - corner[0].getBoundingClientRect().height
                        }
                        if (corner.length > 0) {
                            corner[0].classList.add('fix_y');
                            corner[0].style.top = yy + 'px';
                        }
                    }
                }
            }
        },
        fixcol: function (table) {
            var table = table ? table : document.getElementsByClassName('table')[0],
                tab = Helpers.parents(table, '.mode-content')[0],
                cloned = tab.getElementsByClassName('tb_cloned');
            if (typeof table !== 'undefined' && cloned.length == 0) {
                var tb = Helpers.find(table, '.table__body')[0],
                    t_r = Helpers.find(tb, '.table__row_line'),
                    tc_clone = document.createElement('div');
                tc_clone.classList.add('table__body', 'tb_cloned');
                for (var i = 0; i < t_r.length; i++) {
                    var td = Helpers.find(t_r[i], '.table__cell')[0],
                        sizes = td.getBoundingClientRect(),
                        data = t_r[i].dataset.rowType;

                    td = td.cloneNode(true);
                    var tr = document.createElement('div');
                    tr.classList.add('table__row', 'table__row_line');
                    if (t_r[i].classList.contains('table__row_sub')) {
                        tr.classList.add('table__row_sub');
                    }
                    if (typeof data !== 'undefined') {
                        tr.dataset.rowType = data;
                    }
                    if (t_r[i].classList.contains('disabled')) {
                        tr.classList.add('disabled');
                    }
                    tc_clone.append(tr);
                    td.style.width = sizes.width - 32 + "px";
                    td.style.height = sizes.height - 32 + "px";
                    if (i == 0) {
                        var par = tab.querySelectorAll('.table > .table__head');
                        tc_clone.style.top = par[0].getBoundingClientRect().height + 1 + 'px';
                    }
                    if (td.querySelectorAll('.init').length > 0) {
                        var ini = td.querySelectorAll('.init');
                        for (var y = 0; y < ini.length; y++) {
                            ini[y].classList.remove('init');
                        }
                    }
                    tr.append(td);
                ...
                var table_wrapp = tab.getElementsByClassName('table__wrapp')[0];
                table_wrapp.onscroll = function () {
                    var table_bound = table.getBoundingClientRect();
                    var xx = table_wrapp.scrollLeft;
                    if (window.innerWidth < table_bound.width) {
                        tc_clone.classList.add('fix');
                        tc_clone.style.left = xx + 'px';
                        var corner = tab.getElementsByClassName('table__corner');
                        if (corner.length > 0) {
                            corner[0].classList.add('fix_x');
                            corner[0].style.left = xx + 'px';
                        }
                    } else {
                        var corner = tab.getElementsByClassName('table__corner');
                        if (corner.length > 0) {
                            corner[0].classList.remove('fix_x');
                        }
                        tc_clone.classList.remove('fix');
                    }
                }
            } else if (cloned.length > 0) {
                var corner = tab.getElementsByClassName('table__corner');
                if (corner.length > 0) {
                    var w = tab.querySelectorAll('.table__wrapp > .table > .table__head:not(.th_cloned) > .table__row:first-child > .table__cell:first-child')[0].getBoundingClientRect().width;
                    var tds = tab.querySelectorAll('.table__wrapp > .table > .table__body.tb_cloned > .table__row > .table__cell:first-child');
                    corner[0].childNodes[0].style.width = w + 'px';
                    for (var i = 0; i < tds.length; i++) {
                        tds[i].style.width = w - 32 + 'px';
                        tds[i].style.left = 0 + 'px';
                    }
                }
            }
        },
        fixcorner: function (table) {
            var table = table ? table : document.getElementsByClassName('table')[0],
                tab = Helpers.parents(table, '.mode-content')[0],
                corner = tab.getElementsByClassName('table__corner');
            if (typeof table !== 'undefined' && corner.length == 0) {
                var corner = document.createElement('div');
                corner.classList.add('table__corner');
                var th = tab.getElementsByClassName('table__head')[0];
                var tc = Helpers.find(th, '.table__cell')[0];
                corner.append(tc.cloneNode(true))
                var tc_bound = tc.getBoundingClientRect();
                if (browser == 'safari') {
                    corner.children[0].style.width = tc_bound.width + 0 + "px";
                } else {
                    corner.children[0].style.width = tc_bound.width + "px";
                }
                corner.children[0].style.height = tc_bound.height + 1 + "px";
                table.appendChild(corner);
            }
        },
        colheight: function (table) {
            var table = table ? table : document.getElementsByClassName('table')[0],
                tab = Helpers.parents(table, '.mode-content')[0],
                tc = tab.getElementsByClassName('table__cell');
            for (var i = 0; i < tc.length; i++) {
                var tcp = Helpers.parents(tc[i], '.table__cell');
                if (tcp.length > 1) {
                    var paddings = parseFloat(window.getComputedStyle(tcp[tcp.length - 1], null).getPropertyValue('padding-top')) + parseFloat(window.getComputedStyle(tcp[tcp.length - 1], null).getPropertyValue('padding-bottom'));
                    var height = tcp[tcp.length - 1].clientHeight - paddings;
                    tc[i].style.height = height + "px";
                }
            }
            /* var sub_row = table.querySelectorAll('.table__body .table__row');
             for (var k = 0; k < sub_row.length; k++) {
             var tds = sub_row[k].querySelectorAll('.table__cell');
             for (var t = 0; t < tds.length; t++){
             if (tds.length == '3') {
             var w = parseFloat(sub_row[k].getBoundingClientRect().width / 3).toFixed(3);
             console.log(w)
             tds[t].style.width = w + 'px';
             }
             }
             }*/
        },
